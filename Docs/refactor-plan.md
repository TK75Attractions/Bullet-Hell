...
作成: 2026-07-02 / 対象コミット: 8886a99 / **全フェーズ完了(2026-07-02、タグ refactor-p0-done〜refactor-p5-done)**

**完了サマリ**: P0(ゴールデン+テスト土台) → P1(無害化+25の弾59件復元) → P2(StageChart、リタイム=マーカー1行) → P3(パターン5種+新発射経路+世代ハンドル+QuadOrder分割) → P4(シーク、検証90秒→数秒) → P5(Lunatic-first減算難易度、selectedDifficulty 実効化)。EditMode テスト48本全緑。**新しい制作フローは `Docs/stage-authoring-guide.md` を参照。**

**承認時の修正(2026-07-02)**:
- P3: 弾幕の**発射システム本体(ランタイム)の変更を許可**。使いやすさ優先。ただし旧ステージ(25/captain/mirror/debug)は旧形式のまま動くこと(デュアルパス)
- P5: ボス台詞は延期。難易度の実効化は実施。**「最高難易度(Lunatic)を作成→弾幕を間引いて下位難易度を生成」フローを一級市民として設計**する(減算オーバレイ方式)
...
**P1 実施結果(2026-07-02)**: S1/S2/S3/S5/S6(文字化け修復+死コード)/完了。25の文字化け59件を修復し弾を復元(ゴールデン更新 27→86イベント)。以下は根拠付きで延期:
- S4 世代ハンドル → **P3 に統合**(調査の結果 enemyBullets は append-only で生indexは実害なし。新ランタイム設計時に一括対応)
- S6 未参照バッファのアーカイブ → **P2 に統合**(count=0スポナー参照の扱いを含め、コンパイラ導入時に基準を固めて実施)
- S7 QuadOrder partial分割 → **P3 に統合**(新発射経路でQuadOrderを触る際に同時実施)
- S2 の壊れた index8(rocket) 除去は typeId ずれによるゴールデン差分のため中止(温存+文書化)
...
### P3: パターンライブラリ(ランタイム変更許可済み)

方針更新: パターンは「旧形式JSONへの展開」に縛らず、**新ランタイムイベント(ネイティブのパターンエミッタ)として実装してよい**。StageChart → 新イベント列(パターン参照+パラメータ)を直接実行する経路を追加し、旧ステージ用の legacy ローダーは並存させる。ゴールデンは旧ステージのみを守り、新経路はスクショ/録画パリティで検証する。

StageChart から呼べるパラメータ化ジェネレータ(C#):
...
### P5: 難易度の実効化(Lunatic-first 減算フロー)

方針更新: ボス台詞は延期。難易度は**「Lunatic を正として作成し、間引いて下位を生成する」**フローを一級市民とする。

1. **減算オーバレイ**: StageChart は Lunatic の密度で書く。イベント/パターンに難易度修飾を付けられる:
   - `minDifficulty: normal`(このイベントは NORMAL 以上でのみ出す)
   - `thin: { easy: 1/2, normal: 2/3 }`(パターン内の弾を決定的に間引く: n本ごとに1本残す等)
   - `scale: { easy: { speed: 0.8, count: 0.5 } }`(速度/数の係数)
   - 未指定イベントは全難易度に出る(=既定は「間引かない」)
2. **実行**: ランタイムが `selectedDifficulty` で StageChart イベントをフィルタ/係数適用(コンパイル済み3バリアントではなく実行時フィルタを基本とし、必要ならベイクも可)
3. **プレビュー**: デバッグメニュー/シークに難易度切替を追加し、間引き結果を即確認できるようにする
4. `Clear` を正式イベント化。引き継ぎコードのスロット順を stageDirectoryName 固定マップ化(難易度追加でズレないように)
