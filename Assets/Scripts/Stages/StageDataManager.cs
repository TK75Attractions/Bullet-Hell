...
public class StageDataManager
{
    ...
    private class StageDataJson
    {
        ...
        // Legacy JSON key used by older stage files.
        public List<MultiBulletSpawnerJson> enemySpawners = new();
    ...
    private class MultiBulletSpawnerJson
    {
        ...
        public MultiBulletSpawner ToMultiBulletSpawner()
        {
            ...
            if (bulletChangeClips != null)
            {
                ...
                {
                    if (bulletChangeClips[i] == null) continue;
            ...
            return new MultiBulletSpawner
            {
                ...
                bulletClip = bulletClip != null ? bulletClip.ToBulletClip() : default,
    ...
    private class BulletBufferEmissionJson
    {
        ...
        public BulletBufferEmission ToBulletBufferEmission()
        {
            return new BulletBufferEmission
            {
                ...
                angleOffset = angleOffset != 0f ? angleOffset : angle,
    ...
    [Serializable]
    private class BulletChangeClipJson
    {
        public BulletClipJson clip;
        public float time;//弾が発封E��れてから time 秒後に弾が変化する、E

        public BulletChangeClip ToBulletChangeClip()
        {
            return new BulletChangeClip
            {
                clip = clip != null ? clip.ToBulletClip() : default,
                time = time
            };
        }
    }
    ...
    [Serializable]
    private class BulletClipJson
    {
        public BulletDataJsonDeserializer data;//弾の軌道
        public int number;//弾の数
        public float disRad;//弾の同士のなす见E
        public bool homing;//true なめEHoming する
    ...
    {
        public string clipName; // 呼び出すクリチE�Eの名前
        public int count;//飛�EぁEBullet Buffer の数
        public float interval; // 飛�Eす時間間隁E
        public float time; // 飛�Eす時刻
        public Vector2 pos; //どこから飛�Eすかの座樁E
        public Vector2 originVlc; // originVlc を上書き�E来まぁE時間進展と共に平衡移動可能)
        public float angle; // 飛�Eす角度 (オイラー见E0 <= 角度 < 360)
        public float angleInterval; //Buffer 毎に角度をずらす (1発目めE0, 2発目めE30°, 3発目めE60°... みたいな事が出来めE
    ...
    private StageData ReadStageDataFromDirectory(string name)
    {
        ...
        if (File.Exists(jsonPath))
        {
            try
            {
                ...
                StageDataJson jsonData = JsonUtility.FromJson<StageDataJson>(BulletDataJson.NormalizeLegacyGravityJson(json));
        ...
        foreach (var ext in audioExtensions)
        {
            ...
            if (File.Exists(audioPath))
            {
                try
                {
                    ...
                    {
                        // MP3/OGG/M4A はエチE��タ晁EAssetDatabase、実行時は UnityWebRequest で別途読み込み
#if UNITY_EDITOR
...
#else
                    // 実行時は別送EUnityWebRequest で非同期読み込みが忁E��E
    ...
    private static List<MultiBulletSpawner> ConvertMultiBulletSpawners(StageDataJson jsonData)
    {
        ...
        if ((source == null || source.Count == 0) && jsonData.enemySpawners != null && jsonData.enemySpawners.Count > 0)
        {
            source = jsonData.enemySpawners;
        }
