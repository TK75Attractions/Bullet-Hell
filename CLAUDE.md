...
## 短期的な指示
タイトルのボタンのデザインの改善、文字の位置の修正
石工ステージの改善。具体的には
- 点線をもっとシンプルなデザインに
- 上からブロック落とす攻撃、早すぎる
- ハンマーは外から投げてあてて破裂させる感じにして
- 形態変化のところに表示する 2 + 5 個のブロック、もう少し位置をランダムっぽく表示して。あと重ならないように。
- カッターや破片の色もネイビーにして。あと半透明で最初表示するのをやめて。
- カッターで中央に攻撃するときの速度が遅い
- 形態変化後、ゴーレムの上に老人が乗ってる感じにして