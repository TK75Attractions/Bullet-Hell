...
## 2026-07-12 早朝・浮浪者ビジュアル改善便(Fable 自律・本線再開)

コミット4件: `fbdfc85`(前座UI2件) / `c00a4f0`(浮浪者 回転+レーザー/壁再設計) /
`73d2c3f`(oracle レビュー反映) / `1572503`(post-discord.ps1)。**push 未実施**(検収後)。
Discord 投稿2件(HTTP 200): ①通し録画 `Recordings/vagrant_r9_discord.mp4`(87.3s・
時計焼込・音声付き) ②説明+比較画像4枚。証拠は `.tmp_vagrant/`(cmp_*・r9_f*・
ab_letters_* ほか)と `Recordings/vagrant_r9_20260712.mp4`(フル品質)。

### (1) 今回やったこと

**前座(oracle 小提案2件・確定済み)**
- (0a) プレイHUD の HIT/SCORE 英字 #38C2E0→#42E4FF(+17.5%)、パネル上辺シアン
  ハイライト (0.30,0.68,0.88)→(0.354,0.802,1.0)。リザルトとの明度階層を揃えた
- (0b) 難易度モーダル非選択行に銀枠補強 FrameBoost(枠だけの焼き込み
  `UiButtonStyle.CreateFrameSprite`・BakeBody と同一幾何)。非選択時のみ
  alpha=(1-progress)×0.5、選択行は不変

**本題(浮浪者)— 原典 Main.cpp と参考動画(`Instructions/浮浪者/浮浪者1.mp4`、
TIME表示は拍数・拍0≈video 7.80s)を突き合わせて3点を修正/再設計**
- ①イントロ落下棒の自転を実動化: `useVelocityAngle=False`(描画角に angleSpeed が
  効かない)+`angleSpeed=200.0`(エンジンはラジアン/s 解釈=毎秒31.8回転の
  エイリアシング)の**二重バグで一度も回っていなかった**。True+`math.radians(200)`
  に修正。**棒の位置・appearTime・life は HEAD と完全一致を機械照合**(音ハメ不変)。
  幽霊の三角弾 angleSpeed=120 も同じ単位バグを修正
- ②レーザー再設計(ユーザー承認の原典乖離あり): 一斉発火→原典どおり
  拍96+16i+j に co2={1,1,2,2}/{2,2,3,3} 本のクレッシェンド。予告=**ブラケット型**
  (上下エッジ線+破線中心ガイド・ビート点滅・非致死4拍)→発射0.5拍だけ塗り帯
  (致死)→残光0.5拍。y はブロック単位のレーン分割+レーン内ランダム(タイル状
  ランダム配置、重なりゼロ)。**warn消滅=fire致死開始(d=0)を全32本機械照合**
- ③壁の致死バグ修正+破線化: 旧実装は appearTime 意味論の取り違え
  (appearDuration は appearTime の**前**の点滅窓・当たり判定は time>=appearTime
  から)で、**第1ブロックが予告ゼロで出現と同時に致死**だった。原典準拠の
  サブ波 time1={0,1.5,3}×co1 本数+48セグメント破線帯(0.00375拍/個の左→右の
  走り・隣接行は半ピッチ交互)・予告4拍→致死1拍に再設計
- **非致死予告の正準手法を確立**: appearTime を寿命より後ろに置くと、生涯
  ビート点滅([appearStart,life) が全て点滅窓)のまま一度も致死にならない
- oracle(GPT-5.5 Pro・browser)レビュー: **総合GO**。採用2点=中心ガイドの
  破線化(実線だと点滅ピークで細レーザー発射中に誤読)+壁セグメント継ぎ目の
  半ピッチ交互(格子感解消)。不採用1点=リング左右対称の位相ずらし(オーブ
  着弾との同期・音ハメを崩すため)

### (2) 検証結果

- コンパイル0エラー・**EditMode 11/11 緑**・vagrant.json(chart)は不変。
  既存 dirty(Fonts SDF ほか)には未接触
- JSON 機械監査: イントロ60本の位置/時刻 HEAD 一致=0差分、laser warn↔fire
  d=0(32/32)、非致死のはずが致死になる弾 0、壁セグメント1248・予告の発火前
  食い込み0、破線47本/レーザー×32
- 実経路(select→whiteout→Tutorial(実キー入力)→Playing→Result)の通し録画
  `vagrant_r9_20260712.mp4` から実フレーム抽出で確認:
  t=12.27 棒が各角度に自転 / t=27.82 壁=破線帯で薄い予告と明るい致死が明確・
  隣接行の継ぎ目交互 / t=30.23 致死レーザー1本だけ塗り帯・他5本はブラケット予告
- (0a) 同一フレーム A/B(ラベルの色タグだけ差替): 変化画素は英字行のみ329px、
  (24,40.6,46.7)→(35.2,53.8,58.3)。リムは実フレーム行実測 (14.8,29.2,39.7)→
  (16.7,33.3,44.6)(≈+13%、行ブレンドで希釈)
- (0b) 実モーダルで FrameBoost ON/OFF 同条件撮影: 差分は枠線画素のみ963px、
  枠線上 (50,58,74)→(60,65,80)。`.tmp_vagrant/diffmodal_zoom_cmp.png`

### (3) 未解決と次の一手

- **録画中プレイヤーは無操作(被弾しつつ棒立ち)**。回避を見せる録画が要るなら
  手動プレイ録画が次の一手
- oracle 提案の残り(発射直前0.25拍だけエッジ線を明るく=per-bullet alpha ramp が
  エンジンに無いため未対応)と「全幅直線の五線譜感」は今回スコープ外として保留
- 実経路録画の副産物: **非石工ステージはチュートリアルが必ず入る**
  (`ShouldSkipPreStageTutorial` は石工ハードコード)。録画ルーチンでは
  InputSystem.QueueStateEvent で D/SPACE を注入して突破した(レシピは memory へ)
- `Recordings/vagrant_r8_20260712.mp4`(6.4s・チュートリアルで停止した失敗録画)が
  残っている(削除権限なし)。r8b/r8c は中間デザインの通し録画(参考用)
- 石工の残り弾幕はユーザーの弾幕案待ち(この便では未着手)
...
コミット `d39eb3a`(HUD和文ラベル補正)+`eeddac2`(タイトル退行修正)+
`2c18879`(難易度ボタン縦拡大)+`89d410f`(モーダル行群14px下げ)+本 PROGRESS 便。
**push 未実施**(検収後)。証拠は `.tmp_hudfix/`(cmp_*.png が Before/After 並置)。

### (1) 今回やったこと

**(a) プレイHUD 和文ラベルの上ずれ解消(d39eb3a・最重要指摘)**
- 原因確定: 28ec07d の新HUD(PlayHudController)に TmpAlign 未適用だった
  (他画面は全て適用済み)。**フォント起因ではなくコード修正で解消**
  =フォント差し替え・ライセンス作業は不要
- 帯の全TMPラベル(被弾/スコアのJPラベル・数値・曲名・時刻)に
  CenterInkVertically。非アクティブ空振り罠対応で表示後の Update に
  bool 戻り値の再試行、曲名変更時は再補正。実測補正量: ラベル-3.44px・数値-1.59px

**(b) 難易度ボタンの縦余白+行間拡大(2c18879+89d410f)**
- BarH 124→140(横660据え置き・ラベル40不変)、行間158→180(隙間34→40)
- oracle レビュー(`playui2-vertical-jp-align-review`、A=9.1/10)の残指摘
  「行群がモーダル内で上寄り」→ JSAB モーダルのパネル offset +30→+16

**(c) タイトルメニューの潜在退行を発見・修正(eeddac2)**
- タイトルは起動順(SSManager.Init→TManager.Init)の都合で **Init 済み**
  DefficultyBar の行をクローンしており、6512e8b 以降ボタンが 660x124 に
  引き伸ばされ+マーカースラッシュ二重化していた(実フレームで確認)。
  クローン後に 583x109 へ明示リセット+継承 RowSlash/MarkerSlash 無効化。
  BarH をさらに触る本便の前提修正

### (2) 検証結果(証拠形式)

- コンパイル 0 エラー・**EditMode 11/11 緑**
- HUD: 石工実プレイ t=24/63 の実フレームで、曲名「石工」がパネル上辺寄り→
  光学センター(`cmp_song_3x.png`)、被弾↔数値の中心一致(`cmp_hitcard_3x.png`)、
  スコア(`cmp_scorecard_3x.png`)。oracle「残差1px未満・追加修正不要」
- モーダル: Play実駆動(OpenDifficulty)1080p実フレームで拡大後の全要素
  収まりを確認(`cmp_modal.png`)
- タイトル: 起動実フレームでスラッシュ二重化解消・正規寸法復元(`cmp_title.png`)
- **弾データ不変**(diff は UI 4ファイルのみ)。stageSelectStyle は 1 へ復元済み

### (3) 未解決と次の一手

- **push 未実施**(ローカル ahead 8)。ユーザー検収後に
- REVIEW-NOTES 3項目目「弾幕とHUDの被り」(①レイアウト vs ②弾幕調整)は
  **ユーザー決定待ちのまま**(親の推奨は①)
- oracle の追加提案(未適用・ユーザー判断): HUD の HIT 英字とシアンリムが
  リザルトのシアンより暗い(明度+15〜20%/リムα0.45→0.6 目安)、
  モーダル非選択行の銀枠がやや弱い(明度+10%)
- デバッグ経路の HUD 撮影は PlayHUD がシーン初期 y=+70(実経路の
  AnimateHUDIn が 0 へスライド)のため、**手動表示時は anchoredPosition を
  (0,0) に戻す**(本便で確立。忘れると帯が70px上に切れる)
