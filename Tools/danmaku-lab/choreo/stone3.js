...
// v7b: 点滅（blinkWarn）と同じ理由で、ポップも warn_box では stone_block の奥に描かれて
// 「白い枠」に見えていた。BLINK_TYPE（stone_flash・renderPriority 4）へ切り替えて前面に出す。
//
// 枠に見せないための組み方（1回目の試作で失敗した点）:
//   α は「出た瞬間 (life - appearTime)/0.1 → life で 0」の直線減衰しか作れないため、
//   明るいコマほど長生きになる。拡大したコマを明るく長生きさせると、内側の小さいコマから
//   はみ出した部分だけが白く残って結局「枠」に見えた（1.50→1.00 を等間隔に重ねた版）。
//   そこで拡大はやめ、タイルと同寸(1.00倍)の 白 → 淡ピンク → 地色 の 3 枚を 2/60 秒ずつ
//   ずらして手前へ重ね、拍頭の白から 0.1 秒で地色へ滑らかに落とすだけの構成にした。
//   1.45 倍の短命な白を最背面に足す版も試したが、半透明の白が黒背景に乗ると灰色に見え、
//   タイルの隙間が 1 フレームだけ灰色ににじんだ（v6 の「灰色の枠」と同じ見え方）ので外した。
//   計算値（タイル中心の Y, 0-255）: 255 → 237 → 186 → 174 → 144 → 127 → 93(地色)
const POP_STEP = 1 / 60;           // ずらし幅の基準（1ゲームフレーム相当）
// [スケール(実体タイル比), 色, 出現(クリップ発火からの秒), life(同)] を描画順（先頭が奥）に。
const POP_FRAMES = [
  [1.00, [1.00, 1.00, 1.00, 1.0], 0,             0.100],  // 拍頭の純白フラッシュ
  [1.00, [1.00, 0.58, 0.76, 1.0], 2 * POP_STEP,  0.100],
  [1.00, [1.00, 0.30, 0.58, 1.0], 4 * POP_STEP,  0.100],
];
...
//   出た瞬間 α = hold/0.1 → 自分の持ち時間で 0 へ落ちる直線にしている。
//   v7b: タイル同寸の 3 枚（白→淡ピンク→地色）を時間差で手前へ重ねる。
//   詳細と実測値は POP_FRAMES のコメントを参照。
function tilePop(cells, kind) {
  const items = [];
  cells.forEach(function (cell) {
    const c = cellCenter(cell[0], cell[1]);
    POP_FRAMES.forEach(function (frame) {
      ...
      items.push({
        ...
        appearDuration: 0,
        life: frame[3],            // 出た瞬間 α = (life - t0)/0.1 → life で 0
      });
    });
  });
  return warnClip(items, kind);
