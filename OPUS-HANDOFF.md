...
- ブランチ: `marron/claude-codex`(origin より **42 コミット先行**。push はユーザー確認が必要 → §4)
- 最新コミット: `309aae0` 石工/床消滅: 亀裂ライン(点線)を追加(OPUS-HANDOFF §4-G/JSON のみ)。その前は `cb7c611` 起動リング4→8層(Task A)
...
**推奨着手順(Opus 単独で完結できる順)**: ~~B~~(完了) → ~~A~~(完了) → ~~C~~(完了) → ~~G の亀裂のみ~~(完了) → **D(次はこれ。ただし録画+動画レビューまで完走できるときのみ)**。E/F は録画レビューまで完走できるときのみ。H/I は実装せず提案止まり。G の画面揺れ(カメラ制御)は未実装で別タスク。
