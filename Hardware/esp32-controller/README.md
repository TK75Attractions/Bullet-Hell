...
このファームは `HELLO 2P v2` / `S xx yy` を両方のポートへ同時に出力します。
...
- 起動時: `HELLO 2P v2`
- 最初の入力変化までは2秒ごと: `HELLO 2P v2`
- 入力変化時: `S <P1hex> <P2hex>`
- bit 0=上、bit 1=下、bit 2=左、bit 3=右、bit 4=A（決定/ダッシュ）、bit 5=B（戻る/ポーズ）
- P1: GPIO 4, 5, 6, 7, 8（A）, 15（B）
- P2: GPIO 9, 10, 11, 12, 13（A）, 14（B）

モニタを開いたまま RST を3回押し、毎回 `HELLO 2P v2` が残ることを確認します。
その後 P1/P2 の各方向と A/B ボタンを操作し、`S xx yy` の各 bit が対応どおり変化することを確認します。
例: P1 の A だけ押すと下位バイトが `10`、B だけ押すと `20`、A+B で `30`。

### Unity 側との対応（プロトコルの正）

Unity の入力層（`Assets/Scripts/Managers/InputManager.cs`）はこの `S xx yy` /
`HELLO 2P v2` を正として実装しています。P1 の A/B と方向は 1P プレイヤーへ、B は
全画面共通の「戻る」へ。P2 は入力基盤として `p2*` 状態へ展開され、B はプレイ中の
ポーズのみに配線しています（ゲームロジックの 2P 化は別途）。旧 v1 パーサは bit5 を
無視するだけで後方互換です。
